...
pub mod pallet {
				...
				if let Ok(_guard) = lock.try_lock() {
					{
						...
						if !inputs.is_empty() {
							...
							if let Some((_acct, Ok(()))) = Signer::<T, T::AuthorityId>::any_account(
							)
							.send_signed_transaction(|_acct| Call::submit_price {
								inputs: inputs.clone(),
								timestamp: ts,
							}) {
	...
	#[pallet::type_value]
	pub(super) fn DefaultSupportedSymbols<T: Config>(
	) -> BoundedVec<BoundedVec<u8, ConstU32<32>>, ConstU32<64>> {
		vec![
	...
	pub type LastPrices<T: Config> =
		StorageMap<_, Blake2_128Concat, BoundedVec<u8, ConstU32<32>>, (u128, u64), OptionQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_symbol(origin: OriginFor<T>, symbol: Vec<u8>) -> DispatchResult {
			ensure_root(origin)?;
			let bounded_symbol: BoundedVec<u8, ConstU32<32>> =
			...
			SupportedSymbols::<T>::try_mutate(|symbols| {
				if symbols.contains(&bounded_symbol) {
					...
					symbols
						.try_push(bounded_symbol.clone())
						.map_err(|_| Error::<T>::TooManySymbols)?;
					Ok(())
				}
			})?;
		...
		for src in sources {
			...
			if let Ok(list) = src.fetch(&base_tokens) {
				for (symbol, price) in list {
					...
					entries.push((src.name().as_bytes().to_vec(), symbol.clone(), price, w));
				}
			}
		}
