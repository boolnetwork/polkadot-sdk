...
impl PriceSource for OKX {
	...
	fn fetch(&self, symbols: &[String]) -> Result<PriceList, http::Error> {
		...
		parse_okx_prices(body_str, symbols).ok_or(http::Error::Unknown)
	}
}
...
pub fn parse_okx_prices(
	...
	let data_arr = obj.into_iter().find(|(k, _)| k.iter().copied().eq("data".chars()))?.1;
	...
	for item in items {
		...
		let symbol_str = inst_id_str.split('-').next().unwrap_or(&inst_id_str);
