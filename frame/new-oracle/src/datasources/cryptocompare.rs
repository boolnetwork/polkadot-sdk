...
impl PriceSource for CryptoCompare {
    ...
    fn fetch(&self, symbols: &[String]) -> Result<PriceList, http::Error> {
        ...
        let request =
            http::Request::get(&cryptocompare_url);
        ...
        parse_cryptocompare_prices(body_str)
        .ok_or(http::Error::Unknown)
    }
}

fn parse_cryptocompare_prices(price_str: &str) -> Option<Vec<(BoundedVec<u8, ConstU32<32>>, u128)>>  {
    ...
    for (key_chars, value) in obj.into_iter() {
        ...
        let price_cents = usd_number.integer as u128 * 100 +
            (usd_number.fraction / 10_u64.pow(exp)) as u128;
